//! Migration v019: import fork archive.json entries into upstream archived_at on Instance.
//!
...
fn migrate_profile(profile_dir: &Path) -> Result<()> {
    ...
    let entries: serde_json::Value = match serde_json::from_str(&content) {
        ...
        Err(e) => {
            tracing::debug!(
                "v019: failed to parse {}: {e}, skipping",
    ...
    let mut sessions: serde_json::Value = match serde_json::from_str(&sessions_content) {
        ...
        Err(e) => {
            tracing::debug!(
                "v019: failed to parse {}: {e}, skipping import",
    ...
    if imported > 0 {
        ...
        info!(
            "v019: imported {} archived sessions into {}",
            imported,
            sessions_path.display()
        );
    }
    ...
    info!("v019: removed {}", archive_path.display());
    Ok(())
}
