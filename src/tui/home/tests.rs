...
/// The trashed-session preview placeholder must surface the tool session
/// summary (when tracking is on), the same way the archived and stopped
/// placeholders do. Regression: `render_trashed_preview` previously omitted it.
#[test]
#[serial]
fn trashed_preview_shows_summary() {
    use crate::session::{SummaryState, ToolSessionSummary};
    use crate::tui::styles::load_theme;
    use ratatui::backend::TestBackend;
    use ratatui::Terminal;
    ...
    let storage = Storage::new_unwatched("test").unwrap();
    let mut instance = Instance::new("session0", "/tmp/trashed-preview");
    instance.tool = "claude".to_string();
    instance.tool_session_summary = Some(ToolSessionSummary {
        display_id: "disp-1".to_string(),
        text: "Refactor the auth module".to_string(),
    ...
    instance.trash(); // sets trashed_at -> routes to render_trashed_preview
    let id = instance.id.clone();
    assert!(instance.is_trashed());
    storage
        .update(|i, _| {
            i.push(instance);
            Ok(())
    ...
    .unwrap();
    view.group_by = crate::session::config::GroupByMode::Manual;
    view.trashed_section_collapsed = false;
    view.flat_items = view.build_flat_items();
    view.select_session_by_id(&id);
    view.view_mode = ViewMode::Structured;
    ...
    assert!(
        out.contains("Trash"),
        "trashed preview placeholder should render.\n{out}"
    );
    assert!(
        out.contains("Summary:"),
        "trashed preview must show the Summary label.\n{out}"
    );
    assert!(
        out.contains("Refactor the auth module"),
        "trashed preview must show the summary text.\n{out}"
    );
}
