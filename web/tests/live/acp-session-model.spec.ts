...
async function enableAndSpawn(baseUrl: string, sessionId: string) {
  const enableRes = await fetch(`${baseUrl}/api/sessions/${sessionId}/acp/enable`, { method: "POST" });
  expect(enableRes.ok).toBeTruthy();
  const spawnRes = await fetch(`${baseUrl}/api/sessions/${sessionId}/acp/spawn`, {
    method: "POST",
...
async function waitForReplay(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const replay = (await fetch(`${baseUrl}/api/sessions/${sessionId}/acp/replay?since=0`).then((r) =>
      r.json(),
    )) as Json;
...
function frameEvents(replay: Json, kind: string): Json[] {
  ...
  return frames.map((f) => f.event?.[kind]).filter((e): e is Json => e != null && typeof e === "object");
}
...
base("unstable session_model surfaces as a synthetic model selector", async ({}, testInfo) => {
...
base("selecting the unstable model routes through session/set_model", async ({}, testInfo) => {
  const serve = await spawnAoeServe({
    authMode: "none",
    acp: true,
    workerIndex: testInfo.workerIndex,
  ...
  try {
    ...
    const sawInitial = await waitForReplay(serve.baseUrl, sessionId, (replay) =>
      frameEvents(replay, "ConfigOptionsUpdated").some((e) => modelChoiceValues(e).includes("fake-sonnet")),
    );
    expect(sawInitial).toBe(true);

    const setRes = await fetch(`${serve.baseUrl}/api/sessions/${sessionId}/acp/config-option`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    const sawConfirm = await waitForReplay(serve.baseUrl, sessionId, (replay) =>
      frameEvents(replay, "ConfigOptionsUpdated").some((e) => modelOption(e)?.current_value === "fake-opus"),
    );
...
base("rejected set_model surfaces as ConfigOptionSwitchFailed", async ({}, testInfo) => {
  const serve = await spawnAoeServe({
    authMode: "none",
    acp: true,
    workerIndex: testInfo.workerIndex,
  ...
  try {
    ...
    const sawInitial = await waitForReplay(serve.baseUrl, sessionId, (replay) =>
      frameEvents(replay, "ConfigOptionsUpdated").some((e) => modelOption(e) != null),
    );
    expect(sawInitial).toBe(true);
    ...
    const sawFailure = await waitForReplay(serve.baseUrl, sessionId, (replay) =>
      frameEvents(replay, "ConfigOptionSwitchFailed").some(
        (e) => e.config_id === SYNTHETIC_MODEL_ID && String(e.reason).includes("model unavailable"),
      ),
    );
