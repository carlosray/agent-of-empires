...
async function cycleAxisTo(toggle: import("@playwright/test").Locator, target: string) {
  for (let i = 0; i < 3; i++) {
...
base.describe("sidebar nested repo+group axis (#1720)", () => {
  base("nests user groups inside the repo block", async ({ page }, testInfo) => {
    ...
    try {
      ...
      await expect(
        repo.locator("[data-testid='sidebar-nested-subgroup'] [data-group-id='__ungrouped__']"),
      ).toBeVisible();

      // Every session stays visible, now nested under its subgroup.
      await expect(page.locator("[data-testid='sidebar-session-row']")).toHaveCount(4);
    } finally {
      await serve.stop();
    }
  });
