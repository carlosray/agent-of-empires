...
import { MALFORMED_CUSTOM_THEME_TOML, seedCustomTheme } from "../../helpers/theme";
...
base("malformed custom theme TOML does not break the dropdown", async ({ page }, testInfo) => {
  ...
  try {
    ...
    await expect
      .poll(async () => await page.evaluate(() => document.documentElement.dataset.theme), { timeout: 10_000 })
      .toBe(SWITCH_TO);
  } finally {
    await serve.stop();
  }
