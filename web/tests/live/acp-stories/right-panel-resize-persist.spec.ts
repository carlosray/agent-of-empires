...
base("right panel width persists across reload after dragging the handle", async ({ page }, testInfo) => {
  ...
  try {
    ...
    const storedBefore = await page.evaluate(() => localStorage.getItem("aoe-split-ratio"));
    ...
    await expect
      .poll(async () => await page.evaluate(() => localStorage.getItem("aoe-split-ratio")), {
        timeout: 5_000,
        intervals: [50, 100, 200, 200],
      })
      .not.toBe(storedBefore);

    const storedAfter = await page.evaluate(() => localStorage.getItem("aoe-split-ratio"));
    expect(storedAfter).not.toBeNull();
    const widthAfter = parseInt(storedAfter!, 10);
    expect(widthAfter).toBeGreaterThanOrEqual(280);

    await page.reload();
    const storedReloaded = await page.evaluate(() => localStorage.getItem("aoe-split-ratio"));
