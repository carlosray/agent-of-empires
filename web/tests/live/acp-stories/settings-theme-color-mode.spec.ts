...
base("color-mode change PATCHes but does not dispatch theme-picker-changed", async ({ page }, testInfo) => {
  ...
  try {
    ...
    await expect
      .poll(
        async () => await page.evaluate(() => (window as unknown as { __changedFired?: number }).__changedFired ?? 0),
        { timeout: 5_000 },
      )
      .toBeGreaterThan(0);
    await page.evaluate(() => {
    ...
    const fired = await page.evaluate(() => ({
      picker: (window as unknown as { __pickerFired?: number }).__pickerFired ?? 0,
      changed: (window as unknown as { __changedFired?: number }).__changedFired ?? 0,
    }));
