...
base("DeleteSessionDialog can opt into deleting the worktree", async ({ page }, testInfo) => {
  const serve = await spawnAoeServe({
    ...
    seedFn: ({ home, env }) => {
      ...
      const res = spawnSync(
        resolveAoeBinary(),
        ["add", projectDir, "-t", "story-delete-wt", "-c", "claude", "-w", "feature/x", "-b"],
        { env },
      );
      if (res.status !== 0) {
  ...
  try {
    ...
    const row = page.locator('[data-testid="sidebar-session-row"]').filter({ hasText: "story-delete-wt" }).first();
    ...
    const worktreeCheckbox = page.locator('[data-testid="delete-session-checkbox-worktree"]');
    const branchCheckbox = page.locator('[data-testid="delete-session-checkbox-branch"]');
