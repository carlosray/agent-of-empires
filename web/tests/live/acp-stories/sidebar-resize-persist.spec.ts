...
base("sidebar width persists across reload after dragging the handle", async ({ page }, testInfo) => {
  ...
  try {
    ...
    const storedBefore = await page.evaluate(() => localStorage.getItem("aoe-sidebar-width"));
    ...
    const storedAfter = await page.evaluate(() => localStorage.getItem("aoe-sidebar-width"));
    ...
    await page.reload();
    const storedReloaded = await page.evaluate(() => localStorage.getItem("aoe-sidebar-width"));
