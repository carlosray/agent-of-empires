...
base("iOS dictation commits each partial exactly once, syncs on blur", async ({ page }, testInfo) => {
  ...
  try {
    ...
    const partials = ["open", "open the", "open the diff", "open the diff viewer"];
    ...
    const draftMid = await page.evaluate((k) => localStorage.getItem(k), draftKey);
    ...
    await expect
      .poll(async () => await page.evaluate((k) => localStorage.getItem(k), draftKey), { timeout: 5_000 })
...
base("regular typing after dictation is unaffected", async ({ page }, testInfo) => {
  const serve = await spawnAoeServe({
    authMode: "none",
    acp: true,
    workerIndex: testInfo.workerIndex,
  ...
  try {
    const sessions = await listSessions(serve.baseUrl);
    const seeded = sessions.find((s) => s.title === "story-ios-dictation-then-type");
    if (!seeded) throw new Error("seeded session 'story-ios-dictation-then-type' missing");
    const sessionId = seeded.id;
