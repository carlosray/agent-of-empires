...
base("initial config_option_update from the adapter lands in structured view replay", async ({}, testInfo) => {
...
base("session/set_config_option round-trips and the adapter confirms with a new snapshot", async ({}, testInfo) => {
  const serve = await spawnAoeServe({
    authMode: "none",
    acp: true,
    workerIndex: testInfo.workerIndex,
  ...
  try {
    ...
    const sawInitial = await waitForReplay(serve.baseUrl, sessionId, (json) => json.includes("ConfigOptionsUpdated"));
...
base("user switches reasoning effort to a valid level", async ({}, testInfo) => {
  const serve = await spawnAoeServe({
    authMode: "none",
    acp: true,
    workerIndex: testInfo.workerIndex,
...
base("rejected set_config_option surfaces as ConfigOptionSwitchFailed", async ({}, testInfo) => {
  const serve = await spawnAoeServe({
    authMode: "none",
    acp: true,
    workerIndex: testInfo.workerIndex,
