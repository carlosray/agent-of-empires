  ...
  try {
    ...
    await expect
      .poll(async () => {
        ...
        const frames: Array<{ event?: Record<string, unknown> }> = Array.isArray(replay)
          ? replay
          : (replay?.frames ?? []);
        for (const f of frames) {
          const ups = f.event?.UserPromptSent as { attachments?: Array<{ id: string; kind: string }> } | undefined;
    ...
    const blobRes = await fetch(`${serve.baseUrl}/api/sessions/${sessionId}/acp/attachments/${attachmentId}`);
