...
describe("WorkerRestartingBanner (#1240)", () => {
  it("renders generic restart copy when neither flag is set", () => {
    const { container } = render(<WorkerRestartingBanner agentUnresponsive={false} agentOrphaned={false} />);
    ...
    expect(container.textContent).not.toContain("finished but didn't notify the daemon");
  });

  it("renders cancel-escalation copy when agentUnresponsive is true", () => {
    const { container } = render(<WorkerRestartingBanner agentUnresponsive={true} agentOrphaned={false} />);
    expect(container.textContent).toContain("Agent stopped responding to cancel");
  });

  it("renders silent-orphan copy when agentOrphaned is true", () => {
    const { container } = render(<WorkerRestartingBanner agentUnresponsive={false} agentOrphaned={true} />);
    expect(container.textContent).toContain("Agent finished but didn't notify the daemon");
  });
  ...
  it("prefers agentOrphaned copy when both flags are true", () => {
    ...
    const { container } = render(<WorkerRestartingBanner agentUnresponsive={true} agentOrphaned={true} />);
    expect(container.textContent).toContain("Agent finished but didn't notify the daemon");
    expect(container.textContent).not.toContain("Agent stopped responding to cancel");
  });
});
