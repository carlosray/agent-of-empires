  ...
  return (
    <div>
      ...
      {rows.map((row, i) => {
        ...
        // double-counting.
        const leftCards = row.leftIndex != null ? cardsByEndRow.get(row.leftIndex) : undefined;
        const rightCards =
          row.rightIndex != null && row.rightIndex !== row.leftIndex ? cardsByEndRow.get(row.rightIndex) : undefined;
        const cards = [...(leftCards ?? []), ...(rightCards ?? [])];
        const showForm =
          formRowIndex != null &&
          draftSide != null &&
          (formRowIndex === row.leftIndex || formRowIndex === row.rightIndex);
        const leftActive = rangeStart?.side === "old" && rangeStart.line === row.left?.old_line_num;
        const rightActive = rangeStart?.side === "new" && rangeStart.line === row.right?.new_line_num;
        return (
          <Fragment key={i}>
            <div className="flex">
              <SplitCell
