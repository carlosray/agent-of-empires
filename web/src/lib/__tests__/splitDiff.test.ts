...
function line(type: RichDiffLine["type"], oldNum: number | null, newNum: number | null, content: string): RichDiffLine {
...
describe("buildSplitRows", () => {
  ...
  it("pads the shorter side of an uneven block with placeholders", () => {
    const rows = buildSplitRows(
      hunk([line("delete", 5, null, "old a"), line("add", null, 5, "new a"), line("add", null, 6, "new b")]),
    );
